...
import { AlertTriangle, ShieldAlert, Lock, Clock3, Bot, WifiOff, PenTool, FileText, Database, BookOpen, Mail, RefreshCcw, Upload, X } from 'lucide-react'
...
export function ChatErrorPanel({ error, countdownSeconds, onAction }: ChatErrorPanelProps) {
              ...
              {error.actions.filter(action => !action.auto).map(action => {
                ...
                return (
                  <Button
                    ...
                    className={cn('text-xs', action.variant === 'ghost' && 'hover:bg-muted')}
                  >
